...
`motor_hal.c` 是控制库与板级驱动之间的适配层 我在这里把同一周期的硬件输入保存成快照 再把控制结果提交给硬件

这样拆完以后 Clarke、Park、PID 和 SVPWM 等算法可以继续复用 但当前角度接口仍然包含 `encoder_lines×4` 和增量式编码器计数等假设 移植 MA600 时我还要继续调整
...
这些值由我提前配置 主要包括 PWM 与控制周期、采样和保护参数、电机与编码器参数以及速度和位置分频 它们不是在线辨识结果
